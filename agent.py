...
def main():
        ...
        with open(env_path) as f:
            for line in f:
                if line.startswith("GOOGLE_API_KEY="):
                    os.environ["GOOGLE_API_KEY"] = line.strip().split("=", 1)[1].strip('"').strip("'")
    ...
    while time.time() - start_time < TIMEOUT and attempts < max_attempts:
        ...
        try:
            ...
            try:
                ...
                if valid:
                    ...
                    if lat < best_latency:
                        ...
                        plateaus += 1
                        if plateaus >= 3:
                            print("Optimal schedule confirmed. Stopping early!", flush=True)
                            break
                ...
                feedback = "Your output was not valid JSON. Please provide ONLY valid JSON."
                history.extend([raw_text, feedback])
            ...
            if "429" in error_str:
                wait_time = 65 
                print(f"Hit the RPM limit (429). Sleeping for {wait_time} seconds to clear the bucket...", flush=True)
    ...
    if best_solution:
        ...
        print(f"Successfully wrote best solution to {output_path} with latency {best_latency}")
    else:
        ...
        with open(output_path, "w") as f:
            json.dump({"subgraphs": [], "granularities": [], "tensors_to_retain": [], "traversal_orders": [], "subgraph_latencies": []}, f)
